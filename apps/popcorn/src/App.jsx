...
import PopcornForm from './components/PopcornForm';
import PopcornSummary from './components/PopcornSummary';
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <div>
              <h1 className="heading-xl text-gray-900">
                🍿 Popcorn Production & Packing
              </h1>
              <p className="text-xs sm:text-sm text-gray-600 mt-1">
                Combined Production & Packing System
              </p>
            </div>
            <AuthButton
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 md:py-8">
        {configError ? (
          <div className="card text-center py-12 bg-red-50 border border-red-200">
            ...
            <h2 className="text-2xl font-semibold text-red-900 mb-2">Configuration Error</h2>
            <p className="text-red-700 mb-4 font-medium">{configError}</p>
            <div className="text-sm text-red-600 bg-red-100 p-4 rounded-lg max-w-2xl mx-auto">
              <p className="font-semibold mb-2">To fix this:</p>
              <ol className="text-left space-y-2">
                <li>1. Create a <code className="bg-red-200 px-1 rounded">.env</code> file in <code className="bg-red-200 px-1 rounded">apps/popcorn/</code></li>
          ...
          <div className="card text-center py-12">
            ...
            <p className="text-gray-600 mb-6">Please sign in with your Google account to access the popcorn production system</p>
          </div>
        ) : settingsLoading ? (
          <div className="card text-center py-12">
            <div className="mb-6">
            ...
            <p className="text-gray-600">Fetching configuration from Google Sheets</p>
          </div>
        ) : settingsError ? (
          <div className="card text-center py-12 bg-orange-50 border border-orange-200">
            ...
            <h2 className="text-2xl font-semibold text-orange-900 mb-2">Settings Load Error</h2>
            <p className="text-orange-700 mb-4 font-medium">{settingsError}</p>
          </div>
        ) : (
          <div>
            {/* Tab Navigation */}
            ...
            {activeTab === 'production' ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                  <PopcornSummary refreshTrigger={refreshTrigger} />
                </div>
              </div>
            ) : (
              <div className="max-w-4xl mx-auto">
                <PopcornSummary refreshTrigger={refreshTrigger} fullView={true} />
              </div>
            )}
          </div>
        )}
      ...
      <footer className="bg-white border-t border-gray-200 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <p className="text-center text-sm text-gray-500">
            Popcorn Production & Packing System v1.0 | Real-time Google Sheets Integration
          </p>
        </div>
      </footer>
    </div>
